...
function draw() {
  ...
  if (gameState === "startlevel2") {
    ...
    if (boy.isTouching(rockGroup) || boy.isTouching(logGroup)) {
...
function obstaclesLevel2() {
  ...
  if (frameCount % 300 === 0) {
    console.log("spikes");
    log = createSprite(1200, 365);
    log.addImage("log", logImage);
    log.scale = 0.3;
    log.velocityX = -6;
    logGroup.add(log);
  }
